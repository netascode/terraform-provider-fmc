...
func TestAccFmcManualNatRules(t *testing.T) {
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("fmc_manual_nat_rules.test", "nat_type", "STATIC"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_manual_nat_rules.test", "interface_ipv6", "false"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_manual_nat_rules.test", "fall_through", "false"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_manual_nat_rules.test", "enabled", "true"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_manual_nat_rules.test", "unidirectional", "true"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_manual_nat_rules.test", "description", "description of nat rule"))

	var steps []resource.TestStep
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccFmcManualNatRulesPrerequisitesConfig + testAccFmcManualNatRulesConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccFmcManualNatRulesPrerequisitesConfig + testAccFmcManualNatRulesConfig_all(),
...
const testAccFmcManualNatRulesPrerequisitesConfig = `
...
func testAccFmcManualNatRulesConfig_minimum() string {
...
func testAccFmcManualNatRulesConfig_all() string {
	config := `resource "fmc_manual_nat_rules" "test" {` + "\n"
	config += `	nat_policy_id = fmc_nat_policy._nat_policy_test.id` + "\n"
	config += `	nat_type = "STATIC"` + "\n"
