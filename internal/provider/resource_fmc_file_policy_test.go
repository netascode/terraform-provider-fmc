...
func TestAccFmcFilePolicy(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("fmc_file_policy.test", "file_rules.0.file_type_categories.0.id", "5"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_file_policy.test", "file_rules.0.file_type_categories.0.name", "PDF files"))
